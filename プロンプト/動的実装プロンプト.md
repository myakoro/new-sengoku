...
interface Command {
  name: string
  duration: number      // 行動ターン数（1ターン=週の前半/後半いずれか片方）
  merit: number         // 功績
...
const COMMANDS = {
  ...
  // 盗賊討伐系（ランクD〜S）
  "盗賊討伐（小規模)": { duration: 2, merit: 15, banditRank: "D" },   // 犯罪者1〜2名（旧S）
  "盗賊討伐（中規模)": { duration: 3, merit: 30, banditRank: "C" },   // 小規模盗賊団3〜5名（旧A）
  "盗賊討伐（大規模)": { duration: 4, merit: 40, banditRank: "B", requireRank: "馬上衆" }, // 中規模6〜10名（旧B）
  "盗賊討伐（討伐戦)": { duration: 4, merit: 60, banditRank: "A", requireRank: "馬上衆" }, // 大規模11〜15名（旧C）
  "盗賊討伐（賊軍)": { duration: 8, merit: 80, banditRank: "S" }     // 賊軍20〜25名（旧D）
}
```
...
// ランク別設定（旧S〜Dを反転：Dが最弱、Sが賊軍）
const BANDIT_RANKS = {
  D: { count: [1, 2],  combatRange: [15, 50],  merit: 8,  bossReward: { rice: 0.05,  money: 0.025 } },   // 犯罪者1〜2名（旧S）
  C: { count: [3, 5],  combatRange: [70, 110], merit: 15, bossReward: { rice: 0.075, money: 0.05 } },    // 小規模盗賊団3〜5名（旧A）
  B: { count: [6, 10], combatRange: [150,250], merit: 30, bossReward: { rice: 0.125, money: 0.075 } },   // 中規模盗賊団6〜10名（旧B）
  A: { count: [11,15], combatRange: [300,450], merit: 50, bossReward: { rice: 0.25,  money: 0.125 } },   // 大規模盗賊団11〜15名（旧C）
  S: { count: [20,25], baseCombat: 800,        merit: 80, bossReward: { rice: 0.75,  money: 0.5 } }      // 賊軍20〜25名（旧D）
}
```
