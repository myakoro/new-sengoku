...
| 評定の最小演出 | 中 | 評定を「節目」として感じられるよう、上司/配下の顔アイコン、短い台詞、強調表示、軽量SEなどの最小演出を導入する |
| キャラクター絵（顔アイコン）最小導入 | 中 | 徒士〜馬上衆までの過程を面白くするため、出世/評定/重要通知の一部で顔アイコンを表示できるようにする（固定画像・差し替え可能程度で良い） |
| 公務選択の単調さの解消 | 中 | 徒士/馬上衆相当の公務が「どれを選んでも同じクリック作業」にならないよう、目的/リスク/結果差分を持たせる |
| 所属（組/隊・上司）の可視化 | 中 | メイン画面の左サイドバーで「所属（組/隊）」と「上司（小頭/組頭）」を表示し、ピラミッド構造を把握できるようにする |
...
| 評定：最小演出 | 上司/配下の顔アイコン、短い台詞、強調表示、軽量SEなどにより「節目感」を出す |
| 所属パネル（メイン左） | 所属（組/隊）と上司（小頭/組頭）を表示 |
