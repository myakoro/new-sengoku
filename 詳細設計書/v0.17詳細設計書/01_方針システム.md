...
1. **【外縁探査】（がいえんたんさ）**:
    ...
    - 内容：弱点調査、環境整備、噂の流布。
2. **【潜行手引】（せんこうてびき）**:
    - 対象：ターゲットの側近・門番・侵入ルート。
    - 内容：協力者の抱き込み、鍵の確保、物理的な潜入準備。
3. **【本懐工作】（ほんかいこうさく）**:
    - 対象：ターゲット本人・実行現場。
    - 内容：最終交渉、起請文取り交わし、火薬の設置等。

> [!IMPORTANT]
> **実行の条件**: 第1・第2段階を100%完了させ、第3段階【本懐工作】に突入して初めて、期間終了時の「実行判定」が可能になる。

#### 成功率（進捗）と難易度

| 属性 | 説明 |
|------|------|
| 成功率 | 積み上げた進捗。100%を目指すが難易度補正がかかる |
| 難易度係数 | ターゲットの属性（忠誠、警戒度）により 0.1 〜 1.0 で変動 |
...
export interface PolicyState {
    ...
    plotCategory?: 'person' | 'base' | 'army' | 'clan' // 計略のカテゴリ
