...
| 命令伝達の4段階化 | 大 | MandateStateを拡張し、戦略・任務・部隊・現場の4階層を表現 |
| 進捗ゲージの導入 | 大 | 方針に対する進捗を数値で管理・表示 |
| 手段のアクション化 | 中 | 能力依存の完全義務化とアクション設計の刷新 |
...
- 基本的なゲームループ（週進行、評定周期）
- 戦闘システム（BanditBattleState等）- ただし、準備・追撃フェーズへの接続ロジックを刷新
- 借金システム
- 若党/足軽/馬上衆の管理
- セーブ/ロード機能
...
export interface MandateChain {
    // 第1層：戦略方針（殿様 → 軍団長：大名家の大目標）
    grandStrategy: {
        objective: string           // "東美濃平定" など
        totalBudget: number         // 相当な額（例：5000貫）
        deadline: number
    }
    // 第2層：戦略任務（軍団長 → 部隊長：軍団への具体的使命）
    strategicAssignment: {
        mission: string             // "●●城攻略" / "周辺治安維持" など
        assignedBudget: number      // 軍団予算（例：500貫）
    }
    // 第3層：部隊指揮（部隊長 → 現場責任者：方針配分）
    butaiCommand: {
        policy: PolicyType          // 治安/軍備/計略
        targetMetric: string        // 情報網/士気など
        policyBudget: number        // 部隊予算（例：50貫）
    }
    // 第4層：現場実行（組頭 → プレイヤー：具体的ノルマ）
    genbaExecution: {
        action: ActionType          // 巡察/訓練など
        quota: number               // ゲージ量（例：50）
        entrustedBudget: number     // 現場予算（例：5貫）
    }
}
...
interface GameState {
    ...
    mandateChain: MandateChain | null      // 命令伝達の4段階カスケード
...
export const EVALUATION_CONFIG = {
    ...
    failed: { meritMultiplier: -0.8, bonusRice: 0 },
}
...
4. **MainScreenの変更**
   - 方針表示、進捗（治安度など）＋ゲージ表示 ＋ 4段階命令ツリー表示
