...
| 概念 | 説明 |
|------|------|
| **治安度** | 0〜100の整数。これが目標値（例：60以上で達成） |
| **手段ゲージ** | 0〜100。100になると治安度が+1され、手段ゲージは0にリセット |
...
| 概念 | 説明 |
|------|------|
| **計略成果** | 0〜100の整数。これが目標値（例：20以上で達成） |
| **手段ゲージ** | 0〜100。100になると計略成果が+1され、手段ゲージは0にリセット |

#### 方針ゲージ（手柄の目安）

- 方針ゲージは「計略でどれだけ働いたか」の**目安**であり、進捗効率（計略成果の上がりやすさ）には影響しない
- 進捗効率に影響するのは後述の「手段ゲージ」側
...
初動は成果が出やすいが、同一手段の連打では伸びにくくなる（後半は難しくなる）想定。
...
- **評定時**（4ターン周期：ターン1, 5, 9, 13, ...）
- 前の方針の評価が完了した直後に、次の方針を生成
...
```typescript
function evaluatePolicy(policy: PolicyState): 'excellent' | 'good' | 'normal' | 'failed' {
...
export const POLICY_CONFIG: Record<PolicyType, {
    ...
    '計略': {
        ...
        initialProgress: 0,
        initialGauge: 0,
        baseTargetProgress: 20,
        challengeBonus: 20,
        excellentBonus: 40,
        gaugeMultiplierTable: [
            { maxProgress: 10, multiplier: 1.5 },
            { maxProgress: 30, multiplier: 1.0 },
            { maxProgress: 60, multiplier: 0.7 },
            { maxProgress: 100, multiplier: 0.5 },
        ],
    },
}
...
- 第3段階の命令が直接見える
- 「治安数値を月末までに60まで回復させること。現在は40である」
- 進捗ゲージと目標数値が明確に見える
