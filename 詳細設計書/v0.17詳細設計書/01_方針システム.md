...
- **戦略サイクル（24ターン周期）**：
    - ターン1, 25, 49...
    - **第1層（大名方針）**と**第2層（軍団任務）**が更新される。
    - プレイヤーにとっては「戦局の節目」となる。
- **戦術サイクル（8ターン周期）**：
    - ターン1, 9, 17...
    - **第3層（部隊方針）**が更新される。
    - 部隊長（足軽大将以上）が予算配分を見直すタイミング。
- **実行サイクル（4ターン周期）**：
    - ターン1, 5, 9, 13...
    - **第4層（現場ノルマ）**が更新される。
    - プレイヤーが具体的な作業（ゲージ稼ぎ）の評価を受けるタイミング。
...
export const POLICY_CONFIG: Record<PolicyType, {
    ...
    '計略': {
        ...
        challengeBonus: 20,          // 70%で優秀
        excellentBonus: 40,          // 90%で卓越
...
### 7-2. 軍団長（Gundancho）・部隊長（Butaicho）および上位役職者
- **中評定**（月次レベル：8ターン周期）の命令と配分予算が見える。
...
function getVisiblePolicyInfo(policy: PolicyState, rank: Rank): VisiblePolicyInfo {
    if (rank === '足軽大将' || rank === '組頭' || rank === '侍大将' || rank === '部将' || rank === '家老') {
        // 管理職（部隊長・軍団長）は全情報が見える
        return {
            type: policy.type,
        ...
        // 馬上衆/徒士（小評定受領者：現場実行層）は目標数値は見えないが、自分の委託予算は見え、アプローチ選定が可能
        return {
            type: policy.type,
            progress: policy.progress,
            targetProgress: null,
