...
|------------|----------------|--------------|------|
| **愚直** | ×0 (0貫) | ×0.7 〜 0.9 | 予算を使わず地道に行う。個人の能力依存度が高い。 |
| **並** | ×1.0 (基準) | ×0.8 〜 1.2 | 標準的な予算を投入。安定した成果。 |
| **豪華** | ×1.5 (1.5倍) | ×1.4 〜 1.6 | 資金で成果を底上げする。効率（ROI）は「並」より低め。 |
...
```typescript
// 1. アプローチ選択による倍率決定
// 愚直：cost=0, 成果倍率=random(0.7, 0.9)
// 並  ：cost=0.2, 成果倍率=random(0.8, 1.2)
// 豪華：cost=0.3, 成果倍率=random(1.4, 1.6)

// 2. ゲージ増加
baseGauge = 10 * approachMultiplier
finalGauge = baseGauge // 警備力ゲージへ
...
// 1. アプローチ選択による倍率決定
// 愚直：cost=0, 成果倍率=random(0.7, 0.9)
// 並  ：cost=0.5, 成果倍率=random(0.8, 1.2)
...
- **前提条件**：巡察や情報収集で生成された「盗賊カード」が存在すること。
- 成功すると「平穏度」が大きく上昇する。
...
```typescript
// 1. 討伐対象（カード）の選択
targetCard = selectBanditCard()
if (!targetCard) return // 発見済み盗賊がいない場合は実行不可

// 2. 成功判定
successRate = baseSuccessRate + (player.combat - banditCombat) * 0.3
if (targetCard.hasDetailedInfo) successRate += 10 // 情報収集での発見ボーナス
...
if (success) {
    ...
    abilityMultiplier = 0.8 + (player.combat / 100) * 0.4
    finalGauge = Math.floor(baseGauge * abilityMultiplier) // 平穏度ゲージへ
    removeBanditCard(targetCard.id)
} else {
    finalGauge = 0
}
```
...
|------|---------|--------|--------|---------|-----------|---------|------|
| 巡察 | 5〜10 | 0 | low | none | 1 | 警備力 | 盗賊発見あり |
