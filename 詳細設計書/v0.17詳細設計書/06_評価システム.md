...
function evaluatePolicy(policy: PolicyState): EvaluationResult {
    ...
    // 2. ROI（投資効率）による評価の確定
    // 例：ROIが極めて高い場合、平凡評価でも「優秀」に引き上げ、あるいは追加功績
    const roi = (policy.progress - policy.initialProgress) / policy.consumedBudget
    if (rank !== 'failed' && roi > ROI_THRESHOLD_EXCELLENT) {
        // 効率ボーナス適用
    }
