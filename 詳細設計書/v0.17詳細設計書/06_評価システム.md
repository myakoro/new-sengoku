...
| 卓越 | progress ≥ excellentProgress | 目標を大幅に超過達成（予算を有効活用） |
| 優秀 | progress ≥ challengeProgress | 目標を良好に超過達成 |
| 平凡 | progress ≥ targetProgress | 目標を達成（予算を使い切ってでも可） |
| 未達 | progress < targetProgress | 予算を注ぎ込んでも目標未達 |
...
| 評価 | 成功率（進捗） | 説明 |
|------|-----------|------|
| 卓越 | 90%以上 | ほぼ確実に成功する段階まで根回しした |
| 優秀 | 70%以上 | 高い確率で成功する段階まで工作した |
| 平凡 | 50%以上 | 最低限の目標（五分五分）を達成 |
| 未達 | 50%未満 | 工作不足で成功の見込みが低い |

> [!NOTE]
> **実行判定と手柄**: 
> 評価グレードは「積み上げた成功率」で確定するが、実行そのものの成否（引き抜きの成功等）はターゲットの難易度が影響する。
> 「無理なものは無理（成功率キャップ）」というリアリティを維持しつつ、困難な状況下で高い進捗を稼ぐこと自体は評価対象となる。
...
| 優秀 | ×1.5 | 功績が1.5倍 |
| 平凡 | ×1.0 | 功績が等倍 |
| 未達 | ×-0.6 | 功績が-0.6倍（ペナルティ） |

### 3-2. 盗賊討伐における個人功績（番付評価）

盗賊討伐アクションでは、上記の方針全体の評価とは別に、戦闘中の具体的な働きが功績として直接加算される。

#### 立場別の算定ルール

| 立場 | 功績の構成内容 | 説明 |
| :--- | :--- | :--- |
| **総大将** (小頭/ソロ) | 基礎数値 ＋ **全討取数** | 準備の成功は加点されない（勝利のための手段）。結果（数）がすべて。 |
| **中間** (馬上衆) | 小隊討取数 ＋ **準備成功回数** | 小隊長としての統率力と、上司からの命令遂行を両立して評価。 |
| **一兵卒** (徒士) | 個人討取数 ＋ **準備成功回数** | 個人の武勇と、命じられた役割の完遂度で評価。若党は不参加。 |

#### 具体的な加点目安
- **参加報酬**: 5〜10 pt (一律)
- **準備成功**: 3〜5 pt / 回 (指揮官以外)
- **討ち取り（交戦）**: 5 pt / 人 (トドメを刺した個人に付与)
- **討ち取り（追撃）**: 3 pt / 人 (武芸・機動力・マークに基づき分配)
...
function evaluatePolicy(policy: PolicyState): EvaluationResult {
    ...
    if (policy.progress >= policy.excellentProgress) rank = 'excellent'
    else if (policy.progress >= policy.challengeProgress) rank = 'good'
    else if (policy.progress >= policy.targetProgress) rank = 'normal'
    else rank = 'failed'
    
    // 2. 成果最大化の評価（予算の使い切りを推奨）
    // 予算内でどれだけ積み上げたかを評価する。
    // ROI（進捗/コスト）は参考値とするが、予算を余らせるよりも進捗を稼ぐ方を高く評価する。
    const achievementRatio = (policy.progress - policy.initialProgress) / (policy.targetProgress - policy.initialProgress)
    
    if (rank !== 'failed' && achievementRatio > 1.2) {
        // 目標を大幅に超えて積み上げた場合、評価を底上げ
    }
    
    return {
        rank,
        meritMultiplier: EVALUATION_CONFIG[rank].meritMultiplier,
        bonusRice: EVALUATION_CONFIG[rank].bonusRice,
        comment: getEvaluationComment(rank, roi),
    }
}
```
